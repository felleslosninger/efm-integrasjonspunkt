...
Several request parameters are available, described <<statuses-find_request_parameters, here>>.
...
== Find message statuses by message ID

The service is used to search for message statuses by message ID.
The response is paged with a default page size of 10

=== Example 1 - Find all message statuses by message ID
...
[source,http,role="primary"]
.Curl
include::{snippets}/error/no-content/curl-request.adoc[]

[source,http,role="secondary"]
.Request
include::{snippets}/error/no-content/http-request.adoc[]
