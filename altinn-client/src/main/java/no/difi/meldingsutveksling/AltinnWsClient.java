...
public class AltinnWsClient {
    ...
    public List<FileReference> availableFiles() {
        ...
        if (!isNullOrEmpty(properties.getDpo().getMessageChannel())) {
            fileStream = fileStream.filter(f -> f.getSendersReference().getValue().equals(properties.getDpo().getMessageChannel()));
        } else {
            ...
            fileStream = fileStream.filter(f -> !p.matcher(f.getSendersReference().getValue()).matches());
        }
        return fileStream
