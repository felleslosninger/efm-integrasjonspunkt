...
import no.difi.virksert.scope.ProductionScope;
import no.difi.virksert.scope.TestScope;
...
public class BusinessCertificate {
    ...
    public static Validator getValidator(String scope) {
        switch (scope) {
            case "production":
                return getValidator(new ProductionScope());
            case "test":
                return getValidator(new TestScope());
            default: {
                try {
                    return getValidator((Scope) Class.forName(scope).newInstance());
                } catch (Exception e) {
    ...
    public static Validator getValidator(Scope scope) {
        try {
            KeyStoreCertificateBucket keyStoreCertificateBucket = scope.getKeyStore();
            CertificateBucket rootCertificates = keyStoreCertificateBucket.toSimple(scope.getRootCertificateKeys());
            CertificateBucket intermediateCertificates = keyStoreCertificateBucket.toSimple(scope.getIntermediateCertificateKeys());
